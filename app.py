...
def draw_planet(
    ...
    # Strong global temperature tint: colder -> blue, hotter -> red/orange.
    ...
    c0 = np.clip((1.0 - 0.20) * c0 + 0.20 * temp_tint, 0, 1)
    c1 = np.clip((1.0 - 0.25) * c1 + 0.25 * temp_tint, 0, 1)
    c2 = np.clip((1.0 - tint_strength) * c2 + tint_strength * temp_tint, 0, 1)
    c3 = np.clip((1.0 - tint_strength) * c3 + tint_strength * temp_tint, 0, 1)
    c4 = np.clip((1.0 - tint_strength) * c4 + tint_strength * temp_tint, 0, 1)
    c5 = np.clip((1.0 - tint_strength) * c5 + tint_strength * temp_tint, 0, 1)
    colorscale = [
...
with left:
    with st.form("input_form"):
        ...
        # temperature_c = st.number_input("Temperature (C)", value=15.0, step=0.1)
                ...
                #T=lowerbound([stellar_energy]*(1-[albedo])/(16*pi*[Stefan-Boltzmann constant]*(2*distance_au)^2))^(1/4)
    ...
    if submit:
        ...
        if invalid:
            ...
            forcing = 5.35 * log(co2_ppm / 280)
            temp_k = ((stellar_energy * (1 - albedo) + forcing) / (4 * sigma)) ** 0.25
